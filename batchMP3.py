...
print("Archivos WAV a convertir: ", len(wav_files))
print("Archivos OGG a convertir: ", len(ogg_files))

if len(wav_files) != 0 or len(ogg_files):
			...
			if lstFilesConvertir[i].split('.')[0] == lstFilesMP3[j].split('.')[0]:
				...
				if(contador>0):
					...
					for indice in lista:
						...
						smtp.close()
					print("Cambiando estado_mail en el archivo", archivo_original)
					cursor.execute(""" UPDATE "WebConcursos_audiolocutor"
									   SET estado_mail = '1'
									   WHERE estado = 'Convertido'
									   AND archivo_original = '%s' """ %(archivo_original))
					db.commit()
					db.close()
					print(time.strftime("%d/%m/%y"))
					print(time.strftime("%H:%M:%S"))
